...
PCB::PCB(unsigned long size, ID id, Time time) {
	...
	this->TimeToWait = 0;
	this->timeUnBlocked = 2; //glupa vrednsot
...
int PCB::IsTimeUnBlocked() {

if (this->timeUnBlocked == 1)
	return 1;
else
	return 0;
}
void PCB::SetTimeUnBlocked(int x) {
this->timeUnBlocked = x;
